...
import com.demo.booking.product.domain.exception.ProductStockCannotRevertException;
...
@Transactional
public class RevertStockUseCaseImpl implements RevertStockUseCase {
    
    private final ProductStockRepository productStockRepository;
    
    @Override
    public void revertStock(Long productId, LocalDate checkInDate, LocalDate checkOutDate, int quantity) {
        // 1. 일괄 조회
        ...
        // 3. 일괄 비즈니스 로직 적용
        stocks.forEach(stock -> stock.revertHold(quantity));
    ...
    private void validateStockExists(List<ProductStock> stocks, Long productId, LocalDate checkInDate, LocalDate checkOutDate) {
        ...
        if (stocks.size() != expectedDays) {
            throw new ProductStockCannotRevertException();
        }
    }
}